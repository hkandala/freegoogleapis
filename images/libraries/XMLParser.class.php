<?php
    class XMLParser {
        ...
        private static $_defaultListNode = 'result';
        ...
        public static function encode( $data, $root = NULL ) {
            ...
            try {
                ...
                $XMLStr  = "<?xml version=\"" . $version . "\" encoding=\"" . $encode . "\"?>";
        ...
        private static function _addChildren( $element, $data ) {
            foreach ( $data as $key => $value ) {
                ...
                if($isattr) {
                    if(is_array($value)) {
                        ...
                        $element->addAttribute( self::_formatName($attr[1]), self::_formatValue($value) );
                    }
                    continue;
                }
